...
object MemberPickle {
  ...
  val statusFromString = Map() // Map(Learning.toString -> Learning, Accepting.toString -> Accepting)

  def toJson(membership: Membership): String = {
...
//    "{" + s("name") + ":" + s(membership.membership.name) + "," + s("slot") + ":" + membership.slot + "," + s("members") + ":" + ms + "}"
  ...
  def fromJson(jsonString: String): Membership = ???
//  {
