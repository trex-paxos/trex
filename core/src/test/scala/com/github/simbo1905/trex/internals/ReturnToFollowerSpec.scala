...
class UndefinedReturnToFollowerHandler extends ReturnToFollowerHandler[ActorRef] {
  val plog  = NoopPaxosLogging

  def randomTimeout: Long = 1234L

  def nodeUniqueId: Int = 0

  def commit(state: PaxosRole, data: PaxosData[ActorRef], identifier: Identifier, progress: Progress): (Progress, Seq[(Identifier, Any)]) = (progress, Seq.empty)
