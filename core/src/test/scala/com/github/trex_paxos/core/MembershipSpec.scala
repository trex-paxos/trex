...
class MembershipSpec extends WordSpecLike with Matchers {
  ...
  "Memberships " should {
    "not error when no issues" in {
      ClusterConfiguration(Quorum(2, Set(Weight(1,1), Weight(2,1), Weight(3,1))), Quorum(2, Set(Weight(1,1), Weight(2,1), Weight(3,1))), nodes, Some(0L))
    }
    "error if prepare and accepts quorums do not overlap" in {
      try {
        ClusterConfiguration(Quorum(1, Set(Weight(1,1))), Quorum(1, Set(Weight(2,1))), nodes, Some(0L))
    ...
    "error if a network location is missing" in {
      try {
        ClusterConfiguration(Quorum(2, Set(Weight(1,1), Weight(2,1), Weight(3,1))), Quorum(2, Set(Weight(1,1), Weight(2,1), Weight(3,1))), Set(Node(1, addresses)), Some(0L))
